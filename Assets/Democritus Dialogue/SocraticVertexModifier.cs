...
public class SocraticVertexModifier : MonoBehaviour
{
    ...
    /// <summary>
    /// Returns whether all the text has been displayed.
    /// </summary>
    /// <returns></returns>
        ...
        Dialogue_Superclass.DialogueSection currentSection = null)
    {
        ...
        vertexModifier.hasExecutedPostAction = false;
    ...
    private static string SnipParses(string contents, SocraticVertexModifier vertexModifier)
    {
        ...
        {
            parsedSegment.startCharacterLocation -= totalSnips;
            parsedSegment.endCharacterLocation -= totalSnips;
    ...
    private static void UpdateDebugText(TextMeshProUGUI sendInfoTo, TextMeshProUGUI grabInfoFrom, SocraticVertexModifier vertexMod)
    {
        if (sendInfoTo == null)
        {
            return;
        ...
        {
            debugParseInfo += $"{parse.startCharacterLocation}-{parse.endCharacterLocation} Opening: ({parse.openingParse})\n" +
    ...
    private static void IncrementCharCounter(SocraticVertexModifier vertexModifier)
    {
        if (vertexModifier.counter >= vertexModifier.totalVisibleCharacters)
        {
            ...
            {
                if (!vertexModifier.hasExecutedPostAction)
                {
                    ...
                    vertexModifier.hasExecutedPostAction = true;
                }
            }
        ...
        if (vertexModifier.currentBetweenCharacterDelay <= 0)
        {
            ...
            foreach (var parse in vertexModifier.parses)
            {
                ...
                {
                    if (parse.openingParse && parse.startCharacterLocation == vertexModifier.counter && !parse.executedAction)
                    {
                        if (vertexModifier.currentSection != null)
                        {
                            AudioManager.i.StopAllSources(vertexModifier.currentSection.GetDialogueSound(), true);
                    ...
                    else if (parse.openingParse && parse.startCharacterLocation == vertexModifier.counter - 1 && parse.executedAction)
                    {
                        OnPostCharDelay();
                    }
                }
...
//            for (int i = vertexMod.counter - start; i < vertexMod.counter; i++)
    ...
    private static void ScrollInFromY(SocraticVertexModifier vertexMod, TMP_TextInfo textInfo, Vector3[] newVertexPositions)
    {
        //Under construction
    ...
    /// <param name="textInfo"></param>
    /// <param name="newVertexPositions"></param>
    private static void ApplyRichText(TextMeshProUGUI textComponent, SocraticVertexModifier vertexMod, TMP_TextInfo textInfo, Vector3[] newVertexPositions)
    {
    ...
    private static void ApplyRichTextShake(TextMeshProUGUI textComponent, Vector3[] vertexPositionsReadFrom, SocraticAnnotationParse parse, Vector3[] vertexPositionsWriteTo)
    {
        TMP_TextInfo textInfo = textComponent.textInfo;
        ...
        for (int i = parse.startCharacterLocation; i < parse.linkedParse.startCharacterLocation; i++)
        {
            ...
            Matrix4x4 matrix = Matrix4x4.TRS(jitterOffset * CurveScale, Quaternion.Euler(0, 0, UnityEngine.Random.Range(-5f, 5f) * AngleMultiplier), Vector3.one);
    ...
    /// <param name="parse"></param>
    /// <param name="vertexPositionsWriteTo"></param>
    private static void ApplyRichTextWave(TMP_TextInfo textInfo, Vector3[] vertexPositionsReadFrom, SocraticAnnotationParse parse, Vector3[] vertexPositionsWriteTo)
    {
        float wave_speed = parse.ContainsDynamicValue() ? parse.GetDynamicValueAsFloat() : SocraticAnnotation.wave_speed;
        ...
        for (int i = parse.startCharacterLocation; i < parse.linkedParse.startCharacterLocation; i++)
        {
            ...
            float rightOffsetY = SocraticAnnotation.wave_warpTextVerticies
                ? Mathf.Sin(Time.timeSinceLevelLoad * wave_speed +
                  rightVerticesXPos * SocraticAnnotation.wave_freqMultiplier) * SocraticAnnotation.wave_amplitude
                : leftOffsetY;
    ...
    /// <returns></returns>
    static internal List<SocraticAnnotationParse> GetParses(string inputText, int startAt, List<SocraticAnnotationParse> preFilled = null)
    {
            ...
            if (inputText[i] == SocraticAnnotation.parse_startParse)
            {
                ...
                for (int f = i + 1; f < inputText.Length; f++)
                {
                    if (inputText[f] == SocraticAnnotation.parse_endParse)
                    {
                        ...
                        if (contents.Contains(SocraticAnnotation.parse_endParsePair))
                        {
                    ...
                    if (inputText[f] == SocraticAnnotation.parse_inputValueIndicator)
                    {
